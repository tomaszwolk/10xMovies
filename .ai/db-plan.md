...
### `users`
Provided by Supabase Auth. All user-owned tables will have a foreign key relationship to `auth.users(id)`.
...
- **Owner-only Access**: RLS policies will be enforced to ensure users can only access their own data in `user_platform`, `user_movie`, and `ai_suggestion_batch`. An `event` record with a non-null `user_id` is also owner-only.
- **Admin/System Access**: The `integration_error_log` table will be accessible only by a privileged role (e.g., `admin` or `service_role`).
- **Read-Only Public Data**: Global tables like `movie`, `platform`, and `movie_availability` will be read-only for authenticated, non-privileged users.
- **Backend Access**: The backend application will connect with a role that respects these RLS policies, not the `service_role`.
